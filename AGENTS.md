...
3. **SQLite path resolution** (`packages/db/src/paths.ts`):
   ...
   - `/home/app/data/finlake.db` if the directory `/home/app` exists (Databricks Apps mounts it; only writable path on the runtime)
   - else `<cwd>/data/finlake.db` (local dev)
